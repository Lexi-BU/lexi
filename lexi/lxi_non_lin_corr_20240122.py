...
for i in range(0, len(row_num)):
    ...
    y = np.append(y, dist_x_cm[i] * np.ones(len(hole_number)))
    x = np.append(x, hole_number * dist_h_cm)

# Add the location of four more holes separately
x = np.append(x, np.array([-0.197, 0.197, 0.984, -0.197]) * 2.54)
y = np.append(y, np.array([-0.591, -0.197, 0.197, 1.378]) * 2.54)
xy = np.array([x, y])
...
plt.figure()
plt.scatter(xy_new[0], xy_new[1], s=5, color="r")
plt.xlabel("x (cm)")
plt.ylabel("y (cm)")
plt.title("Coordinates of the holes in the new coordinate system")
...
plt.xlim(-4, 4)
plt.ylim(-4, 4)
plt.savefig("../figures/coordinates.png", dpi=300)
...
max_count = np.array([])
plt.figure()
for i in range(0, len(xy_new[0])):
    dist = np.sqrt((x - xy_new[0][i]) ** 2 + (y - xy_new[1][i]) ** 2)
    mask = dist <= 0.1
    max_count = np.append(max_count, np.max(counts[mask]))
    # Get the corresponding values of x and y
    x_max = x[mask][np.argmax(counts[mask])]
    y_max = y[mask][np.argmax(counts[mask])]
    # Plot the location of the point with maximum count value
    plt.scatter(x_max, y_max, s=5, color="r")

plt.savefig("../figures/maximum_count.png", dpi=300)
