...
class LEXI:
    ...
    Attributes:
        ...
        background_correction_on: bool
            ...
            applied to the LEXI images. If True, background correction is applied to the final LEXI
            images.
        t_range: list
        ...
        t_integrate: float
            ...
            we integrate over to create the lexi histograms and exposure maps. Default is 600
            seconds.
        ra_range: list
            RA (Right Ascension) range to plot, in degrees. [start RA, end RA]. Default is [0.0,
            360.0].
        dec_range: list
            DEC (Declination) range to plot, in degrees. [start DEC, end DEC]. Default is [-90.0,
            90.0].
        ra_res: float
            RA resolution to plot at, in degrees. Default is 0.1.
        dec_res: float
            DEC resolution to plot at, in degrees. Default is 0.1.
        nbins: int
    ...
    def __init__(self, input_params):
        ...
        if self.interp_method not in ["linear", "index", "values", "pad"]:
            warnings.warn(
                f"Requested integration method '{self.interp_method}' not currently supported; "
        ...
        self.background_correction_on = input_params.get(
        ...
        if type(self.t_range) not in [tuple, list]:
            raise TypeError(
                f"t_range should be a tuple or list; instead got {type(self.t_range)}"
            )
        if len(self.t_range) != 2:
            raise ValueError(
                f"t_range should contain exactly 2 elements; instead got {len(self.t_range)}"
            )
        try:
            # (the 'unit' arg is only used if input is of type int/float, so no type checking is needed here)
            self.t_range = (
                pd.Timestamp(self.t_range[0], unit="s"),
                pd.Timestamp(self.t_range[1], unit="s"),
            )
        except ValueError as err:
            raise ValueError(
                f"Could not process the given t_range: {err}. Check that t_range is in "
        ...
        self.ra_range = input_params.get("ra_range", [0, 360])

        # DEC range to plot, in degrees. [start DEC, end DEC]
        self.dec_range = input_params.get("dec_range", [-90, 90])
        ...
        if input_params.get("nbins") is not None:
            ...
            ra_nbins, dec_nbins = (
                (nbins[0], nbins[1]) if type(nbins) in [tuple, list] else (nbins, nbins)
            )
            if input_params.get("ra_res") or input_params.get("dec_res"):
                print(
                    f"Both (ra_res and/or dec_res) and nbins were specified; ignoring res value and setting "
    ...
    def get_spc_prams(self):
        ...
        dfinterp: pandas dataframe
            Dataframe containing all ephemeris data (time, look direction RA, look direction DEC,
            roll angle, and potentially other columns), sliced to t_range and interpolated to t_step
            using interp_method.
        """
        ...
        # Compute min/max for RA and DEC for default plot ranges
        fov_padding = 5  # degrees
        min_ra = np.min(df['mp_ra']) - fov_padding
        max_ra = np.max(df['mp_ra']) + fov_padding
        min_dec = np.min(df['mp_dec']) - fov_padding
        max_dec = np.max(df['mp_dec']) + fov_padding
        ra_range = [min_ra, max_ra]
        dec_range = [min_dec, max_dec]

        return dfinterp, ra_range, dec_range
    ...
    def get_exposure_maps(self, save_maps=True):
        ...
        spc_df, ra_limits, dec_limits = self.get_spc_prams()
        ...
        ra_arr = np.arange(ra_limits[0], ra_limits[1], self.ra_res)
        dec_arr = np.arange(dec_limits[0], dec_limits[1], self.dec_res)
        ...
        try:
            ...
            ra_start = ra_limits[0]
            ra_stop = ra_limits[1]
            dec_start = dec_limits[0]
            dec_stop = dec_limits[1]
            ra_res = self.ra_res
    ...
    def get_sky_backgrounds(self):
        ...
        spc_df: pandas dataframe
            Dataframe containing all ephemeris data (time, look direction RA, look direction DEC,
            roll angle, and potentially other columns), sliced to t_range and interpolated to
            t_step using interp_method.
        t_range: list
            Time range to consider. [start time, end time]. Times can be expressed in the following
            formats:
                1. A string in the format 'YYYY-MM-DDTHH:MM:SS' (e.g. '2022-01-01T00:00:00')
        ...
        dec_res: float
            DEC resolution to plot at, in degrees. Default is 0.1.
        save_maps: bool
            If True, save the exposure maps to a file.

        Returns:
        --------
        sky_backgrounds: list
            List of ROSAT sky background images, corrected for LEXI exposure time. Shape:
            num-images.ra-pixels.dec-pixels, where num-images depends on t_range and
            t_integrate, ra-pixels depends on ra_range and ra_res, and dec-pixels depends on
            dec_range and dec_res.
        """
        # Get exposure maps
        ...
        rosat_df.index = np.linspace(ra_arr[0], ra_arr[1], 100)
        rosat_df.columns = np.linspace(dec_arr[0], dec_arr[1], 100)
        # Reindex to include desired RA/DEC indices (but don't throw out old indices yet; need for interpolation)
        desired_ra_idx = np.arange(ra_arr[0], ra_arr[1], self.ra_res)
        desired_dec_idx = np.arange(dec_arr[0], dec_arr[1], self.dec_res)
        ...
        ).interpolate(method=self.interp_method, axis=1)
    ...
    def get_lexi_images(self):
        ...
        Returns:
        --------
        bgcorr_histograms: list
            List of LEXI science histograms. Shape: num-images.ra-pixels.dec-pixels,
            where num-images depends on t_range and t_integrate, ra-pixels depends on ra_range and
            ra_res, and dec-pixels depends on dec_range and dec_res.
        """
        ...
        photons = pd.DataFrame(
            {
                "ra_J2000_deg": [355, 355, 335, 333, 330, 333, 335],
        ...
        ra_grid = np.arange(ra_arr[0], ra_arr[1], self.ra_res)
        dec_grid = np.arange(dec_arr[0], dec_arr[1], self.dec_res)
